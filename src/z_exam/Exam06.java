...
public class Exam06 {
	...
	public static void main(String[] args) {
		...
		[답]
		인스턴스변수: weapon과 armor(모든 병사가 동일한 값을 가져야 하므로)
		메서드: weaponUp()과 armorUp()
		...
		[답]
		a: 자동으로 초기화되지 않는다.
		e: 힙영역은 인스턴스(객체)가 저장되며, 지역변수는 call stack에 저장된다.
		...
		[답]
		a: static변수의 초기화에 인스턴스 변수를 사용할 수 없다.
		b: static메서드에서 인스턴스 변수를 사용할 수 없다.
		d: static메서드에서 인스턴스 메서드를 사용할 수 없다.
		...
		class Exercise6_19
		{
			public static void change(String str) {
				str += "456"; // 값을 바꾼순간 기존의 str과 주소가 달라진다.
			}
			public static void main(String[] args)
			{
				String str = "ABC123";
		...
		ABC123
		After change:ABC123
		change(str)에서 str의 값을 복사하여 change메서드 안의 String str에 간다.
		(str이 아닌 다른 참조형이면 변해서 ABC123456이 됐을텐데
		 str은 값이 변하면 새로운 주소를 생성한다.)
